...
def extract_chromagram(file_path, plot=False):
    ...
    if plot:
        ...
        axs[1].set_title('STFT Spectrogram (Linear Frequency)')
        axs[1].set_ylabel('Frequency (Hz)')
        axs[1].set_xlabel('Time (s)')
        fig.colorbar(img1, ax=axs[1])


        img4 = axs[2].imshow(chromagram, origin='lower', aspect='auto',
                             extent=[t[0], t[-1], 0, 12])
        axs[2].set_yticks(np.arange(12))
        axs[2].set_yticklabels(['C', 'C#', 'D', 'D#', 'E', 'F',
                                'F#', 'G', 'G#', 'A', 'A#', 'B'])
        axs[2].set_title('Chromagram')
        axs[2].set_xlabel('Time (s)')
        axs[2].set_ylabel('Pitch Class')
        fig.colorbar(img4, ax=axs[2])
